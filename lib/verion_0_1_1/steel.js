...
function steelConfig(opt){
    ...
    cssPostfix_perfix = buildParam.cssPostfix_perfix,
...
function tasks(){
    ...
    function dealSrc(isDebug) {
        ...
        var jsFilter_not_lib = $.filter([jsBase + '**/*.js', "!"+ jsBase + 'lib/**/*.js'], {restore: true});

        var jsFilter_of_lib = $.filter([jsBase + 'lib/steel.js', jsBase + 'lib/**/*.js'], {restore: true});
            jsFilter_of_lib._concatTo_ = jsBase + 'lib/lib.js';
        ...
        ////
        //处理js
        result = result.pipe(jsFilter_not_lib)
            .pipe($.steelWrapAmd())
            .pipe($.steelAmdConcat({moduleBasePath: jsBase}));
        result = result.pipe(jsFilter_not_lib.restore);
